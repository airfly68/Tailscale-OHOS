# 从一个例子开始-一次开发，多端部署 - 华为HarmonyOS开发者

本章通过一个天气应用，介绍一多应用的整体开发过程，包括UX设计、工程管理及调试、页面开发等。

## UX设计

### UX设计

本示例中的天气应用包含主页、管理城市和添加城市三个页面，其中主页中又包含菜单和更新间隔两个弹窗，基本业务逻辑如下所示。

### UX设计

“一多”建议从最初的设计阶段开始就拉通多设备综合考虑。考虑实际智能终端设备种类繁多，设计师无法针对每种具体设备各自出一份UX设计图。“一多”建议从设备应用窗口宽度和窗口高宽比两个维度，将设备划分类别。

### UX设计

以应用窗口宽度为判断条件，建议划分为5个区间：

### UX设计

设备类型 | 窗口宽度（vp）
超小设备 | (0, 320）
小设备 | [320, 600)
中设备 | [600, 840)
大设备 | [840, 1440)
超大设备 | [1440, +∞)

### UX设计

根据应用窗口的高宽比进行判断，建议划分为3个区间：

### UX设计

设备类型 | 高宽比
小设备 | (0, 0.8)
中设备 | [0.8, 1.2)
大设备 | [1.2, +∞)

### UX设计

说明 vp是virtual pixel（虚拟像素）的缩写，是常用的长度单位。

### UX设计

直板机、双折叠、平板对应于小设备、中设备及大设备，本示例以这三种设备场景为例，介绍不同设备上的UX设计。天气主页在不同设备上的设计图如下所示。

### UX设计

| 小设备 | 中设备 | 大设备
主页 | | |

### UX设计

另外，大设备中天气主页还允许用户开启或者隐藏侧边栏。

### UX设计

开启侧边栏 | 隐藏侧边栏
|

### UX设计

从天气应用在各设备上的UX设计图中，可以观察到如下UX的一些“规律”：

### UX设计

- 在不同的屏幕宽度下，应用的整体风格基本保持一致。 在相近的屏幕宽度范围内，应用的布局基本不变；在不同的屏幕宽度范围内，应用的布局有较大差异。 应用在小屏幕下显示的元素，是大屏幕中显示元素的子集。 考虑到屏幕尺寸及显示效果，大屏幕中可以显示的元素数量一定不少于小屏幕。 为充分利用屏幕尺寸优势，大屏幕可以有其独有的元素或设计（如本示例中的侧边栏）。

### UX设计

如此，既在各设备上体现了UX的一致性，也在各设备上体现了UX的差异性，从而既可以保障各设备上应用界面的体验，也可以最大程度复用界面代码。

## 工程管理及调试

### UX设计 > 工程管理及调试

完成UX设计后，接下来需要考虑如何将设计转化为实际可运行的工程，在本文 中，将详细介绍一多的工程创建及管理等，本小节仅介绍最基础的工程创建及多设备预览调试。 多设备工程部署

## 工程创建

### UX设计 > 工程管理及调试 > 工程创建

一多应用的工程创建过程，与传统应用并无较大差异。只需在工程创建过程中，注意在“Device Type”选项中勾选所有该应用期望运行的目标设备类型，保证后续该应用可以在所有目标设备上正确安装即可。

## 预览调试

### UX设计 > 工程管理及调试 > 预览调试

在代码开发过程中，可以开启预览器，并打开“Multi-profile preview”开关，实时观察应用在不同设备下的表现。

### UX设计 > 工程管理及调试 > 预览调试

特别的，还可以点击“+ New Profile”按钮，新增自定义预览器。

## 页面开发

### UX设计 > 页面开发

天气应用中涉及较多的页面和弹窗，本小节以天气主页为例，简单介绍不同设备下的页面实现思路。

### UX设计 > 页面开发

观察天气主页在不同设备上的UX设计图，可以进行如下设计：

### UX设计 > 页面开发

- 将天气主页划分为9个基础区域，如： 基础区域9仅在大设备上显示（需要在大设备上展开侧边导航栏），基础区域1-8虽然在各设备上始终展示但其尺寸及区域内的布局基本保持不变（区域8需要向下滑动设备至底部，下方示例图不做展示），可以结合 能力以 的形式分别实现这9个基础区域。 自适应布局 自定义组件 小设备 中设备 大设备 主页 基础区域1-8之间的布局在不同设备上有较大差异，可以使用响应式布局中的 能力实现组件间的布局效果。 栅格 展开和隐藏侧边栏的功能可以通过 来实现。侧边栏是大设备上独有的，借助响应式布局中的 能力，控制仅在大设备上展示侧边栏即可。 侧边栏组件 媒体查询

## 主页基础区域

### UX设计 > 页面开发 > 主页基础区域

天气主页中的9个基础区域介绍及实现方案如下表所示。

### UX设计 > 页面开发 > 主页基础区域

编号 | 简介 | 实现方案
1 | 标题栏 | 自适应布局拉伸能力。
2 | 天气概览 | Row和Column组件，并指定其子组件按照主轴起始方向对齐或居中对齐。
3 | 每小时天气 | 自适应布局延伸能力。
4 | 每日天气 | 自适应布局延伸能力。
5 | 空气质量 | Canvas画布组件绘制空气质量图，并使用Row组件和Column组件控制内部元素的布局。
6 | 生活指数 | 自适应布局均分能力。
7 | 日出日落 | Canvas画布组件绘制日出日落图。
8 | 应用信息 | Row和Column组件，并指定其子组件居中对齐。
9 | 侧边导航栏 | 综合运用自适应布局中的拉伸能力、占比能力和延伸能力。

### UX设计 > 页面开发 > 主页基础区域

天气主页涉及的内容较多，因篇幅限制，本小节仅介绍区域3（每小时天气）的实现。

### UX设计 > 页面开发 > 主页基础区域

延伸能力是指容器组件内的子组件，按照其在列表中的先后顺序，随容器组件尺寸变化显示或隐藏。随着可用显示区域的增加，用户可以看到的“每小时天气”信息也不断增加，故“每小时天气”可以通过延伸能力实现，其核心代码如下所示。

### UX设计 > 页面开发 > 主页基础区域

```
{ , getHoursData, , } ; import Forecast MyDataSource Style from 'common' @Preview @Component struct { export default HoursWeather : [] = ( ); hoursData Forecast getHoursData 0 : < > = ( . ); @State hoursDataResource MyDataSource Forecast new MyDataSource this hoursData ( ) : = ; @StorageLink 'curBp' curBp string 'lg' // ... ( ) { build // Implement extensibility capability through list component. () { List ( . , { LazyForEach this hoursDataResource ( ) => hoursItem: Forecast () { ListItem . (hoursItem, this HoursWeatherItem . === ? . + : . ) this curBp 'lg' Style WEATHER_ITEM_WIDTH 2 Style WEATHER_ITEM_WIDTH } }, . (hoursItem) + index) ( ) => hoursItem: Forecast, index: number JSON stringify } . ( ) width '100%' . ( . ) height Style CARD_HEIGHT . ( . ) borderRadius Style NORMAL_RADIUS . ( . ) backgroundColor Style CARD_BACKGROUND_COLOR . ( . ) listDirection Axis Horizontal } }
```

## 城市天气详情

### UX设计 > 页面开发 > 城市天气详情

天气主页右侧的城市天气详情由区域1-8组成，区域1（标题栏）始终固定在页面顶部，区域2-8在不同设备下的布局不同且可以随页面上下滚动。本小节介绍如何实现城市天气详情中区域2~8的布局效果。

### UX设计 > 页面开发 > 城市天气详情

设备屏幕可能无法一次性显示区域2-8的所有内容，故需要在外层增加滚动组件（即Scroll组件）以支持上下滚动。不同设备下区域2-8的相对位置一共有三套不同的布局，可以借助响应式布局中的 实现这一效果。本示例中将栅格在不同场景下分别划分为4列、8列和12列，区域2-8在不同场景下的布局如下表所示。 栅格

### UX设计 > 页面开发 > 城市天气详情

小设备 | 中设备 或 大设备（侧边栏显示状态） | 大设备（侧边栏隐藏状态）
| |

### UX设计 > 页面开发 > 城市天气详情

说明 为提升用户体验，大设备侧边栏隐藏状态下，每日天气与空气质量的相对顺序发生了改变。可以通过调整GridCol栅格子组件的order属性，实现目标效果。

### UX设计 > 页面开发 > 城市天气详情

```
; import AirQuality from './AirQuality' ; import HoursWeather from './HoursWeather' ; import IndexHeader from './IndexHeader' ; import IndexEnd from './IndexEnd' ; import LifeIndex from './LifeIndex' ; import MultidayWeather from './MultidayWeather' ; import SunCanvas from './SunCanvas' { , } ; import CityListData Style from 'common' @Component struct { export default HomeContent : ; @Prop showSideBar boolean ( ) : [] = []; @StorageLink 'titleText' titleText string ( ) : = ; @StorageLink 'swiperIndex' swiperIndex number 0 : = ; @State headerOpacity number 1 cityListData?: ; CityListData : = ; index number 1 : = (); scroller Scroller new Scroller ( ) { build ( . ) { Scroll this scroller ({ GridRow : { columns : , xs 4 : , sm 4 : , md 8 : . ? : lg this showSideBar 8 12 }, : { : . , : . }, gutter x Style GRID_GUTTER y Style GRID_GUTTER : { : . } breakpoints reference BreakpointsReference WindowSize }) { // Weather overview. ({ GridCol : { span : , xs 4 : , sm 4 : , md 8 : . ? : lg this showSideBar 8 12 }, : order 0 }) { ({ : . !. , : . }) IndexHeader headerDate this cityListData header index this index . ( . ) opacity this headerOpacity } // Hourly weather. ({ GridCol : { span : , xs 4 : , sm 4 : , md 8 : lg 8 }, : order 1 }) { ({ : . !. }) HoursWeather hoursData this cityListData hoursData } // Daily weather. ({ GridCol : , span 4 : { order : , xs 2 : , sm 2 : , md 2 : . ? : lg this showSideBar 2 3 } },) { ({ : . !. }) MultidayWeather weekData this cityListData weekData } // Air quality. ({ GridCol : , span 4 : { order : , xs 3 : , sm 3 : , md 3 : . ? : lg this showSideBar 3 2 } }) { ({ : . !. , : . !. }) AirQuality airData this cityListData airData airIndexData this cityListData airIndex } // Living index. ({ : , : }) { GridCol span 4 order 4 ({ : . !. }) LifeIndex lifeData this cityListData suitDate } // Sun canvas. ({ : , : }) { GridCol span 4 order 5 () SunCanvas } // Index end. ({ GridCol : { span : , xs 4 : , sm 4 : , md 8 : . ? : lg this showSideBar 8 12 }, : order 6 }) { () IndexEnd } } } // ... } }
```

## 主页整体实现

### UX设计 > 页面开发 > 主页整体实现

综合考虑各设备下的效果，天气主页的根节点使用侧边栏组件：

### UX设计 > 页面开发 > 主页整体实现

- 小设备和中设备既不展示侧边栏，也不提供控制侧边栏显示和隐藏的按钮。 大设备默认展示侧边栏，同时提供控制侧边栏显示和隐藏的按钮。

### UX设计 > 页面开发 > 主页整体实现

另外主页右侧的城市天气详情，支持左右滑动切换城市，可以使用 实现目标效果： Swiper组件

### UX设计 > 页面开发 > 主页整体实现

- 小设备和中设备开启Swiper组件的导航点，引导用户通过左右滑动切换不同城市。 大设备中用户通过点击侧边栏中的城市列表即可高效的切换不同城市，此时需要关闭Swiper组件的导航点。

### UX设计 > 页面开发 > 主页整体实现

```
; import HomeContent from './HomeContent' ; import IndexTitleBar from './IndexTitleBar' ; import SideContent from './SideContent' { , , getBg, getCityListWeatherData, } ; import CityListData Style Logger from 'common' // ... @Entry @Component struct { Home // ... : = ; @State curBp string 'md' : [] = (); @State cityListWeatherData CityListData getCityListWeatherData : = ; @State showSideBar boolean false // ... ( ) { build ( . ) { SideBarContainer SideBarContainerType Embed // Left sidebar. ({ : $showSideBar }) SideContent showSideBar . ( ) height '100%' // Right content area. () { Column // Title bar. ({ : $showSideBar }) IndexTitleBar showSideBar . ( ) height 56 // Weather details. () { Swiper ( . , { ForEach this cityListWeatherData ( ) => item: CityListData, index: number ({ : . , : item, : index }) HomeContent showSideBar this showSideBar cityListData index }, . (item) + index) ( ) => item: CityListData, index: number JSON stringify } . ( ) id 'swiper' . ({ : . , : . }) padding left Style NORMAL_PADDING right Style NORMAL_PADDING . ( { onChange => index . = index; this swiperIndex . ( , . ); AppStorage setOrCreate 'swiperIndex' this swiperIndex }) // Disable navigation dots on lg width breakpoint. . ( . !== ? () indicator this curBp 'lg' new DotIndicator . ( . ) : selectedColor Color White false ) . ( . ) index this swiperIndex . ( ) loop false . ( ) width '100%' . ( ) layoutWeight 1 } . ( ) height '100%' } . ( ) height '100%' . ( ) sideBarWidth '33.3%' . ( ) minSideBarWidth '33.3%' . ( ) maxSideBarWidth '33.3%' . ( ) showControlButton false . ( . ) showSideBar this showSideBar . ( . ) backgroundImageSize ImageSize Cover . ( ( . [ . ]. . )) backgroundImage getBg this cityListWeatherData this swiperIndex header weatherType } // ... }
```

### UX设计 > 页面开发 > 主页整体实现

最终，天气首页的运行效果如下图所示。

### UX设计 > 页面开发 > 主页整体实现

小设备 | 中设备 | 大设备（隐藏侧边栏） | 大设备（显示侧边栏）
| | |

## 功能开发

### UX设计 > 功能开发

应用开发不仅包含应用页面开发，还包括应用后端功能开发以及服务器端开发等。服务器端开发不在本文的讨论范围内，本小节仅介绍多设备上应用功能开发的注意事项。

### UX设计 > 功能开发

如前文所示，本示例的目标运行设备是小设备、中设备和大设备，对应实际的设备类型为默认设备和平板等。这些设备运行的都是标准系统，其系统能力一致，所以无需做特别考虑。但是在超小设备（对应的实际设备类型为智能穿戴设备等）上，考虑CPU、内存、硬盘等硬件限制，往往会对系统进行裁剪。如果在应用后端功能开发时调用当前系统没有的能力，就可能会引发异常。

### UX设计 > 功能开发

通常有两种方式解决上述问题：

### UX设计 > 功能开发

- 在应用安装包中描述其需要的系统能力，保证本应用仅被分发和安装到可以满足其诉求的系统中。 在使用特定系统能力前，通过canIUse接口判断系统能力是否存在，进而执行不同的逻辑。

### UX设计 > 功能开发

在本文的 章节中，将详细展开介绍。 多设备功能开发

## 示例代码

### UX设计 > 示例代码

- 一多天气

### UX设计 > 示例代码

如果您有其他疑问，您也可以通过开发者社区问答频道来和我们联系探讨。

### UX设计 > 工程管理及调试 > 工程创建

Device

### UX设计 > 工程管理及调试 > 工程创建

Type

### UX设计 > 工程管理及调试 > 预览调试

Multi

### UX设计 > 工程管理及调试 > 预览调试

New

### UX设计 > 工程管理及调试 > 预览调试

Profile

### UX设计 > 页面开发 > 主页基础区域

Forecast

### UX设计 > 页面开发 > 主页基础区域

MyDataSource

### UX设计 > 页面开发 > 主页基础区域

Style

### UX设计 > 页面开发 > 主页基础区域

Preview

### UX设计 > 页面开发 > 主页基础区域

Component

### UX设计 > 页面开发 > 主页基础区域

HoursWeather

### UX设计 > 页面开发 > 主页基础区域

State

### UX设计 > 页面开发 > 主页基础区域

StorageLink

### UX设计 > 页面开发 > 主页基础区域

Implement

### UX设计 > 页面开发 > 主页基础区域

List

### UX设计 > 页面开发 > 主页基础区域

LazyForEach

### UX设计 > 页面开发 > 主页基础区域

ListItem

### UX设计 > 页面开发 > 主页基础区域

HoursWeatherItem

### UX设计 > 页面开发 > 主页基础区域

WEATHER_ITEM_WIDTH

### UX设计 > 页面开发 > 主页基础区域

JSON

### UX设计 > 页面开发 > 主页基础区域

CARD_HEIGHT

### UX设计 > 页面开发 > 主页基础区域

NORMAL_RADIUS

### UX设计 > 页面开发 > 主页基础区域

CARD_BACKGROUND_COLOR

### UX设计 > 页面开发 > 主页基础区域

Axis

### UX设计 > 页面开发 > 主页基础区域

Horizontal

### UX设计 > 页面开发 > 城市天气详情

AirQuality

### UX设计 > 页面开发 > 城市天气详情

IndexHeader

### UX设计 > 页面开发 > 城市天气详情

IndexEnd

### UX设计 > 页面开发 > 城市天气详情

LifeIndex

### UX设计 > 页面开发 > 城市天气详情

MultidayWeather

### UX设计 > 页面开发 > 城市天气详情

SunCanvas

### UX设计 > 页面开发 > 城市天气详情

CityListData

### UX设计 > 页面开发 > 城市天气详情

HomeContent

### UX设计 > 页面开发 > 城市天气详情

Prop

### UX设计 > 页面开发 > 城市天气详情

Scroller

### UX设计 > 页面开发 > 城市天气详情

Scroll

### UX设计 > 页面开发 > 城市天气详情

GridRow

### UX设计 > 页面开发 > 城市天气详情

GRID_GUTTER

### UX设计 > 页面开发 > 城市天气详情

BreakpointsReference

### UX设计 > 页面开发 > 城市天气详情

WindowSize

### UX设计 > 页面开发 > 城市天气详情

Weather

### UX设计 > 页面开发 > 城市天气详情

GridCol

### UX设计 > 页面开发 > 城市天气详情

Hourly

### UX设计 > 页面开发 > 城市天气详情

Daily

### UX设计 > 页面开发 > 城市天气详情

Air

### UX设计 > 页面开发 > 城市天气详情

Living

### UX设计 > 页面开发 > 城市天气详情

Sun

### UX设计 > 页面开发 > 城市天气详情

Index

### UX设计 > 页面开发 > 主页整体实现

IndexTitleBar

### UX设计 > 页面开发 > 主页整体实现

SideContent

### UX设计 > 页面开发 > 主页整体实现

Logger

### UX设计 > 页面开发 > 主页整体实现

Entry

### UX设计 > 页面开发 > 主页整体实现

Home

### UX设计 > 页面开发 > 主页整体实现

SideBarContainer

### UX设计 > 页面开发 > 主页整体实现

SideBarContainerType

### UX设计 > 页面开发 > 主页整体实现

Embed

### UX设计 > 页面开发 > 主页整体实现

Left

### UX设计 > 页面开发 > 主页整体实现

Right

### UX设计 > 页面开发 > 主页整体实现

Column

### UX设计 > 页面开发 > 主页整体实现

Title

### UX设计 > 页面开发 > 主页整体实现

Swiper

### UX设计 > 页面开发 > 主页整体实现

ForEach

### UX设计 > 页面开发 > 主页整体实现

NORMAL_PADDING

### UX设计 > 页面开发 > 主页整体实现

AppStorage

### UX设计 > 页面开发 > 主页整体实现

Disable

### UX设计 > 页面开发 > 主页整体实现

DotIndicator

### UX设计 > 页面开发 > 主页整体实现

Color

### UX设计 > 页面开发 > 主页整体实现

White

### UX设计 > 页面开发 > 主页整体实现

ImageSize

### UX设计 > 页面开发 > 主页整体实现

Cover
